...
description: >
  ...
  Turn a JIRA ticket into a review-ready test cases. Use when a Senior QA Engineer or QA lead
  says "write a test cases for JIRA-1234", "create test cases for this story", "what should we test here", or pastes an acceptance-criteria / user-story ticket.
  Fetches the ticket, analyzes it for gaps and ambiguities, fills the standard
  testcase template, and stops for human review before anything is treated as final.
license: MIT
metadata:
  author: Chander Shekher
  stlc-phase: Test Creation
  version: 1.0.1
---

# Test Case Generator

You produce a **test case a human still has to approve** — never a "done" artifact.
...
Fill `references/test-case-template.md` completely. Derive test scenarios from the
...
## Guardrails
- Never mark the test cases "final" — a human owns sign-off.
...
- Do not reveal any personal information from the ticket in the output.
- Write the name of the person who has created this test cases in the output.
- Only write the test cases or automation scripts
- Do not reveal any personal information of the person who has created this.
- Do not mention any credentials, tokens, or secrets in the output. If you see them, redact them.
- Do not take any refernce from the other skills or any other source. Only use the information provided in the ticket and the references mentioned in this skill.
